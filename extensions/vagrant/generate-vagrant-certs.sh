...
for dir in files/ca files/certs; do
  if [ -d "$dir" ]; then
    continue
  fi
...
if [ ! -e files/ca/Vagrant_Root_CA.key ] && [ ! -e files/ca/Vagrant_Root_CA.crt ]; then
    ...
    openssl req -newkey ec -pkeyopt ec_paramgen_curve:P-256 -nodes \
      -subj '/O=ansible_collections/OU=community.zabbix/CN=Vagrant Root CA' -days 7300 \
...
if [ ! -e files/ca/Vagrant_Intermediate_CA.key ] && [ ! -e files/ca/Vagrant_Intermediate_CA.crt ]; then
    ...
    openssl req -newkey ec -pkeyopt ec_paramgen_curve:P-256 -nodes \
        -subj '/O=ansible_collections/OU=community.zabbix/CN=Vagrant Intermediate CA' -days 3650 \
        -extensions v3_ca \
          -addext 'keyUsage = critical, digitalSignature, cRLSign, keyCertSign' \
...
for hostname in ${!certificate_days[*]}; do
  ...
  # Don't renew certificates that expire in more than 15 days.
  ...
  openssl req -newkey ec -pkeyopt ec_paramgen_curve:P-256 -nodes \
    -subj "/O=ansible_collections/OU=community.zabbix/CN=${hostname}.vagrant.local" -days ${certificate_days[$hostname]} \

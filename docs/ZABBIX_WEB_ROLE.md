...
* `zabbix_web_packages`: A list of packages to install for the zabbix web frontend.
...
* `zabbix_php_fpm_listen`: The absolute path to a socket file or ipaddress:port combination on which the zabbix PHP-FPM pool needs to listen. If none are provided, defaults are used.
* `zabbix_php_fpm_conf_listen_owner`: Default: php-fpm user. Controls ownership and access to php-fpm pool socket
* `zabbix_php_fpm_conf_listen_group`: (Optional) Controls group access to php-fpm pool socket
...
* `zabbix_web_php_fpm_max_requests`: The maximum number requests a single worker will process before being killed and respawn. - default: 200
